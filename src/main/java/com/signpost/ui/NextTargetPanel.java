...
public class NextTargetPanel extends PluginPanel
{
	...
	// Not a CardLayout - that sizes both views to the TALLER one (the Suggest list), and
	...
	/** {@code itemManager}/{@code skillIconManager} are for icons only; either may be null (tests), which renders placeholders. */
	...
	public void render(Advice advice)
	{
		...
		// The group ironman shared storage line, same freshness rule as the bank; blank for every other account type.
	...
	/** Task-only Advice publication: no route/card rebuild on every Slayer kill. */
	public void renderSlayer(Advice advice)
	{
		if (!SwingUtilities.isEventDispatchThread())
		{
			throw new IllegalStateException("NextTargetPanel.renderSlayer must run on the EDT");
		}
		com.signpost.snapshot.SlayerState slayer = advice.getSnapshot().getSlayer();
		slayerLabel.setText(slayer.isActive()
			? SuggestPanel.wrap("Slayer: " + slayer.getRemaining() + " " + slayer.getTaskName()
				+ " left (" + slayer.getMaster() + "), " + String.format(java.util.Locale.ROOT, "%,d", slayer.getPoints()) + " pts")
			: "");
	}
