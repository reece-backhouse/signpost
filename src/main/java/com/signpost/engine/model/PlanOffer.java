 ...
 * doesn't meet dropped, in plan order; {@code alternativeSteps} is the same per
...
public class PlanOffer
{
	...
	List<BringItemStatus> items;
	List<List<BringItemStatus>> alternativeItems;

	public PlanOffer(GatheringPlan plan, boolean meetsRequirements, List<String> missing, List<String> steps,
		List<List<String>> alternativeSteps)
	{
		this(plan, meetsRequirements, missing, steps, alternativeSteps, List.of(), List.of());
	}

	public PlanOffer(GatheringPlan plan, boolean meetsRequirements, List<String> missing, List<String> steps,
		List<List<String>> alternativeSteps, List<BringItemStatus> items, List<List<BringItemStatus>> alternativeItems)
	{
		this.plan = plan;
		this.meetsRequirements = meetsRequirements;
		this.missing = List.copyOf(missing);
		this.steps = List.copyOf(steps);
		this.alternativeSteps = List.copyOf(alternativeSteps);
		this.items = List.copyOf(items);
		this.alternativeItems = List.copyOf(alternativeItems);
	}
}
