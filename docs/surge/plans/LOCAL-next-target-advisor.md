...
1. **Gradle project** `plugin/`: `settings.gradle`, `build.gradle` from the
...
2. **No-network gate** `NoNetworkTest`: walks `src/main/java`, fails if any
...
3. **`Snapshot`** (immutable, Lombok `@Value @Builder`): `accountHash`,
...
4. **`SnapshotCollector.collect(Client, ItemManager, BankMemory)`**: pure
   read of client state; asserts `client.isClientThread()`. Not unit-tested
   (needs a client); verified in dev client.
5. **`AccountStore`**: `load(accountHash)` / `save(AccountData)` as JSON
   ...
   `AccountStore` API.
6. **`NextTargetPlugin`** wiring per spec rulings 9, 18, 19, 23:
...
7. **`NextTargetPanel`** header only: account type, quests done/total,
...
8. **Dev-client checklist** (commit body): A1 type shown; A2 quest count
...
1. **kb-build project**: `package.json` (type module, scripts `build-kb`,
   ...
   `wiki.ts` exports.
2. **`lua.ts`**: parser for the subset used by `Module:Questreq/data` and
...
3. **`questreq.ts`**: Lua → `{name, skills[{skill, level, boostable,
   ironmanOnly}], prereqs[]}`; drops `Name_of_quest`.
4. **`questPages.ts`**: wikitext → `{items[{name, qty}], questPoints,
...
5. **`quests.ts` resolver** (ruling 13): RuneLite `Quest` name list is
...
6. **`diaries.ts`**: per diary page, parse tier tables → tasks
...
7. **`emit.ts`**: stable key order, `generatedAt` = max revision timestamp
   (ruling 21). Test: two emits of the same input are byte-identical.
8. **Plugin side** `kb/` model (Gson, immutable), `KnowledgeBase.load()`
...
9. **Snapshot → diary task completion**: `DiaryProgress.completed(snapshot,
   kb)`. Test: given varp 1176 = 0b0110, tasks 1 and 2 complete.
10. **Dev-client check** (ruling 20): temporary debug log per tier
...
4. **`FocusPicker`** (E3 rule lands in S5; here only the tree): panel
...
5. **Wiring**: `Advice { statuses, focus }`; `Prefs` from store; panel mode
   switch. Refresh triggers per ruling 23 (F4). Focus recompute on any
   snapshot (E6 prerequisite).
6. **Dev-client checklist**: pick SotE, tree matches wiki reqs; complete a
...
1. **`milestones.json`** hand-drafted (~40 entries incl. slayer targets and
...
2. **kb-build `materials.ts`**: for every item id referenced anywhere in the
...
3. **`RoutePlanner.route(skill, fromXp, toXp, bank, kb)`** per ruling 15.
...
4. **`ShortfallResolver`** (C7): best method at current sim level →
   `(item, have, need)` + sources filtered by account type; craft sources
   recurse one level. Test: toadflax chain from the ticket.
5. **`FocusPicker.next(status)`** E3 rule a→d. Tests: one per branch.
6. **Boss gear any-of** (C8). Test.
7. **Perf test** (F5): maxed-account fixture, all goals, assert engine < 1 s.
8. **Panel**: route checklist, shortfall + sources, wiki links (F3).
9. **Dev-client checklist**: SotE focus shows Herblore route from real bank.
